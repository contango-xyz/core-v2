...
contract GranaryMoneyMarketTest is Test {
    ...
    IERC20 internal rewardsToken = IERC20(0xfD389Dc9533717239856190F42475d3f263a270d);

    function setUp() public {
        env = provider(Network.Optimism);
        env.init(112_800_061);

        contango = address(env.contango());

        sut = env.deployer().deployGranaryMoneyMarket(env, IContango(contango));
        pool = AaveMoneyMarketView(address(sut)).pool();
        env.createInstrument(env.erc20(WETH), env.erc20(USDC));

        positionId = env.encoder().encodePositionId(Symbol.wrap("WETHUSDC"), MM_GRANARY, PERP, 1);
    ...
    function testInitialise_InvalidExpiry() public {
        ...
        sut = env.deployer().deployGranaryMoneyMarket(env, IContango(contango));
        positionId = env.encoder().encodePositionId(Symbol.wrap("WETHUSDC"), MM_GRANARY, PERP - 1, 1);
    ...
    function testLifeCycle_HP() public {
        ...
        assertEqDecimal(rewardsToken.balanceOf(contango), 466.222220673534129027e18, rewardsToken.decimals(), "rewardsToken.balanceOf");
        ...
        vm.prank(contango);
        sut.claimRewards(positionId, lendToken, borrowToken, contango);
        assertEqDecimal(rewardsToken.balanceOf(contango), 1398.666662020602387982e18, rewardsToken.decimals(), "rewardsToken.balanceOf");
    }
