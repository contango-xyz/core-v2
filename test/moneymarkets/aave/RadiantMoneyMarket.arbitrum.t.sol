...
contract RadiantMoneyMarketArbitrumTest is Test {
    ...
    function setUp() public {
        ...
        sut = env.deployer().deployRadiantMoneyMarket(env, IContango(contango));
        pool = AaveMoneyMarketView(address(sut)).pool();
        env.createInstrument(env.erc20(WETH), env.erc20(DAI));

        positionId = env.encoder().encodePositionId(Symbol.wrap("WETHDAI"), MM_RADIANT, PERP, 1);
    ...
    function testInitialise_InvalidExpiry() public {
        ...
        sut = env.deployer().deployRadiantMoneyMarket(env, IContango(contango));
        positionId = env.encoder().encodePositionId(Symbol.wrap("WETHDAI"), MM_RADIANT, PERP - 1, 1);
