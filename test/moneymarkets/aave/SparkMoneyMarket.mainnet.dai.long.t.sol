...
contract SparkMoneyMarketMainnetDAILongTest is Test {
    ...
    function testInitialise_InvalidExpiry() public {
        ...
        sut = env.deployer().deploySparkMoneyMarket(env, IContango(contango));
        positionId = env.encoder().encodePositionId(Symbol.wrap("WETHDAI"), MM_SPARK, PERP - 1, 1);
    ...
    function testInitialise_InvalidMoneyMarketId() public {
        ...
        sut = env.deployer().deploySparkMoneyMarket(env, IContango(contango));
        positionId = env.encoder().encodePositionId(Symbol.wrap("WETHDAI"), MM_AAVE, PERP, 1);
