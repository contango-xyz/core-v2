...
contract SparkMoneyMarketMainnetUSDCShortTest is Test {
    ...
    function setUp() public {
        ...
        positionId = env.encoder().encodePositionId(Symbol.wrap("USDCWETH"), MM_SPARK, PERP, 1);
        vm.startPrank(contango);
        sut.initialise(positionId, env.token(USDC), env.token(WETH));
        vm.stopPrank();
    ...
    function testInitialise_InvalidExpiry() public {
        ...
        sut = env.deployer().deploySparkMoneyMarket(env, IContango(contango));
        positionId = env.encoder().encodePositionId(Symbol.wrap("USDCWETH"), MM_SPARK, PERP - 1, 1);
    ...
    function testInitialise_InvalidMoneyMarketId() public {
        ...
        sut = env.deployer().deploySparkMoneyMarket(env, IContango(contango));
        positionId = env.encoder().encodePositionId(Symbol.wrap("USDCWETH"), MM_AAVE, PERP, 1);
    ...
    function testInitialise_NoEMode() public {
        ...
        assertEq(lendToken.allowance(address(sut), psm.gemJoin()), type(uint256).max, "lendToken allowance");
        assertEq(env.token(DAI).allowance(address(sut), address(psm)), type(uint256).max, "PSM allowance");
        assertEq(env.token(DAI).allowance(address(sut), address(env.token(SDAI))), type(uint256).max, "USDC allowance");
