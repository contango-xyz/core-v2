...
contract AgaveMoneyMarketGnosisDAILongTest is Test {
    ...
    IERC20 internal rewardsToken = IERC20(0x388Cae2f7d3704C937313d990298Ba67D70a3709);
    ...
    function setUp() public {
        ...
        sut = env.deployer().deployAgaveMoneyMarket(env, IContango(contango));
        pool = AaveMoneyMarketView(address(sut)).pool();
        env.createInstrument(env.erc20(WETH), env.erc20(DAI));

        positionId = env.encoder().encodePositionId(Symbol.wrap("WETHDAI"), MM_AGAVE, PERP, 1);
    ...
    function testInitialise_InvalidExpiry() public {
        ...
        sut = env.deployer().deployAgaveMoneyMarket(env, IContango(contango));
        positionId = env.encoder().encodePositionId(Symbol.wrap("WETHDAI"), MM_AGAVE, PERP - 1, 1);
    ...
    function testLifeCycle_HP() public {
        ...
        assertEqDecimal(rewardsToken.balanceOf(contango), 0.012744493607585775e18, rewardsToken.decimals(), "rewardsToken.balanceOf");
        ...
        vm.prank(contango);
        sut.claimRewards(positionId, lendToken, borrowToken, contango);
        assertEqDecimal(rewardsToken.balanceOf(contango), 0.038233480822757326e18, rewardsToken.decimals(), "rewardsToken.balanceOf");
    }
