...
contract SparkMoneyMarketMainnetUSDCLongTest is Test {
    ...
    function setUp() public {
        ...
        positionId = env.encoder().encodePositionId(Symbol.wrap("WETHUSDC"), MM_SPARK, PERP, 1);
    ...
    function testInitialise_InvalidExpiry() public {
        ...
        sut = env.deployer().deploySparkMoneyMarket(env, IContango(contango));
        positionId = env.encoder().encodePositionId(Symbol.wrap("WETHUSDC"), MM_SPARK, PERP - 1, 1);
    ...
    function testInitialise_NoEMode() public {
        ...
        assertEq(env.token(DAI).allowance(address(sut), address(pool)), type(uint256).max, "DAI allowance");
        assertEq(env.token(DAI).allowance(address(sut), address(psm)), type(uint256).max, "PSM allowance");
        assertEq(borrowToken.allowance(address(sut), psm.gemJoin()), type(uint256).max, "USDC allowance");
