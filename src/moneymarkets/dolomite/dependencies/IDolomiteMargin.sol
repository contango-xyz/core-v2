...
import "./ILinearStepFunctionInterestSetter.sol";
...
interface IDolomiteMargin {
    ...
    function getLiquidationSpreadForPair(uint256 heldMarketId, uint256 owedMarketId) external view returns (D256 memory);
    ...
    function getMarketIdByTokenAddress(IERC20 token) external view returns (uint256);
    function getMarketInterestRate(uint256 marketId) external view returns (Rate memory);
    function getMarketInterestSetter(uint256 marketId) external view returns (ILinearStepFunctionInterestSetter);
    ...
    function getMarketTokenAddress(uint256 marketId) external view returns (IERC20);
    function getMarketTotalPar(uint256 marketId) external view returns (TotalPar memory);
    function getMarketWithInfo(uint256 marketId) external view returns (Market memory, Index memory, Price memory, Rate memory);
    ...
    function ownerWithdrawUnsupportedTokens(IERC20 token, address recipient) external returns (uint256);
